...
Git.prototype.resolve = function(giturl) {
    if (Git.isGitoliteUrl(giturl)) {
        // remove any protocol
        giturl = giturl.slice(giturl.indexOf('gitolite'));
    } else {
...
Git.isGitoliteUrl = function(giturl) {
    return (giturl.indexOf(GITOLITE_PREFIX) > -1);
};
...
Git.parseUrl = function(giturl) {
    ...
    } else if (!Git.isGitoliteUrl(giturl)) {
        return null;
    }
